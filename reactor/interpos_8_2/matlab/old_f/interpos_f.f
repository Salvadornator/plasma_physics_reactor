...
c   of at least 2 dimensions, except if all 8 arguments are given
c
...
c%OS      pointer(iptr_xin,xin)
c%OS      pointer(iptr_yin,yin)
c%OS      pointer(iptr_xout,xout)
c%OS      pointer(iptr_sig,sig)
      real*8 xin(1000), yin(1000), xout(1000), ybc(6), sig(1000)
      integer kopt, nbc(2)
      real*8 taus, zkopt, znbc(2)
c   pointers for matlab output arguments
      integer*4 pyout, pyoutp, pyoutpp
c%OS      pointer(iptr_yout,yout)
c%OS      pointer(iptr_youtp,youtp)
c%OS      pointer(iptr_youtpp,youtpp)
      real*8 yout(1000), youtp(1000), youtpp(1000)
...
c   added statement for variables
     ...
     +  noutrow, noutcol, nbytes8, ioptder, iflag
c
...
c
              nkopt=10
      open(nkopt,file='kopt.file')
      read(nkopt,*) zkopt,nin
              narray=12
      open(narray,file='array.file')
      read(narray,*) (xin(i),yin(i),i=1,nin)
      nrhs=3
      nlhs=1
c%OS      pkopt=mxGetPr(prhs(1))
c   call mxCopyPtrToInteger4(pkopt, kopt, 1)  ! does not work => use real8
c%OS      call mxCopyPtrToReal8(pkopt, zkopt, 1)
      kopt = abs(zkopt)
      kopt_sign=zkopt
...
c
c%OS      pxin=mxGetPr(prhs(2))
c%OS      ninrow = mxGetM(prhs(2))
c%OS      nincol = mxGetN(prhs(2))
c   input 1D arrays in row or column
c%OS      nin = max(ninrow,nincol)
      nbytes = 8*nin
c%OS      print *,nbytes
c%OSc%OS      iptr_xin = malloc_f(nbytes,infomalloc)
c%OSc%OS      iptr_yin = malloc_f(nbytes,infomalloc)
c%OSc%OS      call mxCopyPtrToReal8(pxin, xin, nin)
c%OS      print *,' nin= ',nin
c%OS      print *,' xin= ',(xin(i),i=1,nin)
c
c   2.3 get yin
c
c%OSc%OS      pyin=mxGetPr(prhs(3))     ! Get yin
c%OSc%OS      call mxCopyPtrToReal8(pyin, yin, nin)
...
c   option was asked, and only 4 input arguments are given. Otherwise the
c   4th argument is assumed to be xout
c
        i4len = max(mxGetM(prhs(4)),mxGetN(prhs(4)))
        if (i4len.eq.1 .and. nrhs.ne.8 .and. inttype.eq.3) then
c   in this case prhs(4) gives the tau value  
          ...
          nout = i4len
          nbytes = 8*nout
c%OS          iptr_xout = malloc_f(nbytes,infomalloc)
...
c   2.7 7th or 8th is sigmain
      nbytes = LENREAL*nin
c%OS      iptr_sig = malloc_f(nbytes,infomalloc)
...
c%OS      iptr_yout = malloc_f(nbytes8,infomalloc)
      ioptder = 0
      if (nlhs .ge. 2) then
        plhs(2) = mxCreateFull(noutrow,noutcol,0)
        pyoutp = mxGetPr(plhs(2))
c%OS        iptr_youtp = malloc_f(nbytes8,infomalloc)
        ioptder = 1
      endif
      if (nlhs .ge. 3) then
        plhs(3) = mxCreateFull(noutrow,noutcol,0)
        pyoutpp = mxGetPr(plhs(3))
c%OS        iptr_youtpp = malloc_f(nbytes8,infomalloc)
        ioptder = 2
      endif
c
...
c%OS      pointer(iptr_pynew,pynew)
      real*8 pynew(1000)
c%OS      pointer(iptr_pyinpp,pyinpp)
      real*8 pyinpp(1000)
c%OS      pointer(iptr_pamat,pamat)
      real*8 pamat(100000)
c%OS      pointer(iptr_pwork,pwork)
      real*8 pwork(10000)
c%OS      pointer(iptr_kwork,kwork)
      integer kwork(10000)
C
...
c
     ...
     +  .OR. NBCRGT.EQ.2) IDAMAT = 3*(IDAMAT-1)+1
      ...
      mdamat = IDAMAT*nin
      nbytes = LENREAL*nin
c%OS      iptr_pynew = malloc_f(nbytes,infomalloc)
c%OS      iptr_pyinpp = malloc_f(nbytes,infomalloc)
      nbytes = LENREAL*mdamat
c%OS      iptr_pamat = malloc_f(nbytes,infomalloc)
      nbytes = LENREAL*(2*nin)
c%OS      iptr_pwork = malloc_f(nbytes,infomalloc)
      nbytes = LENREAL*(5*nin)
c%OS      iptr_kwork = malloc_f(nbytes,infomalloc)
c%OS      PXEXP0 = xin(1) - 1.
c%OS      PXEXPDL= 1.0
      PXEXP0 = ybc(5)
      PXEXPDL= ybc(6)
c%OS      if (infomalloc .eq. 0) then
...
c%OS      pointer(iptr_ftauk,ftauk)
      dimension ftauk(1000)
C
...
C     0. INITIALIZATION
C
c%OS      iptr_ftauk = malloc_f(LENREAL*knin,infomalloc)
c%OS      if (infomalloc .ne. 0) return
      ITAUVAL = 0

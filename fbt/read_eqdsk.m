...
if nargin == 0
  ...
  fnamefull = fname;
  if ~exist(fnamefull)
    disp([fnamefull,' does not exist, prompts for it']); 
    [fname,pathname] = uigetfile([{'EQDSK*;eqdsk*'},{'EQDSK* or eqdsk* files'}; ...
                    {'*EQDSK*;*eqdsk*'},{'*EQDSK* or *eqdsk* files'};{'*'},{'All files'}],'Select an EQDSK file');
    if isequal(fname,0) || isequal(pathname,0); return; end
    fnamefull = fullfile(pathname, fname);
...
if ~isempty(intwhich)
  ...
  % use d|psi-psi_axis| = sigma_Ip sigma_Bp dpsi
  qtofit = sign(eqdskval.ip).*cocos_struct.sigma_rhothetaphi.*(2.*pi).^(1-cocos_struct.exp_Bp).*eqdskval.q;
  psi_incr = abs(eqdskval.psimesh.*(eqdskval.psiedge-eqdskval.psiaxis));
  ...
  if min(sign(eqdskval.b0).*PhitorsignB0) < 0.
...
if docalculateBRetc ~= 1
  %if docalculateBRetc ~= -1; disp('do not calculate BR, BZ, Bphi, etc'); end
  return
end
%disp('do calculate BR, BZ, Bphi, etc')

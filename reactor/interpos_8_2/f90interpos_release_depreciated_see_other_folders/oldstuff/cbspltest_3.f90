    program cbspltest_3
      ...
      write(*,'(a,8I)') '% info_time= ',info_time
      ...
      call random_number(rannumbers)
      do i=1,nin
         xin(i)=3._rkind*TWOPI*real(i-1,rkind)/real(nin-1,rkind)
         yin(i)=sin(xin(i) + rannumbers(i)*zepsilon)**2
      end do
      do i=1,nout
         xout(i)=3._rkind*TWOPI*real(i-1,rkind)/real(nout-1,rkind)
      end do
      !
...
!
      ...
      print *,'];'
      print *,' out=[ '
      write(*,'(1p4e25.15)') (xout(i), yout(i), youtp(i), youtpp(i), i=1,nout)
      print *,'];'
...
! test with periodic conditions
      ...
      nbc(1)=-1
      ybc(1)=xin(nin)-xin(1)
      call cbsplgenp0(xin(1:nin-1),yin(1:nin-1),nin-1,xout,yout,youtp,youtpp,nout, &

program interpostest
  ...
  taus=-1._rkind
  xout(1:nin)=xin
  call interpos(xin,yin,yinspl,yinsplpp,nin,xout,yout,youtp,youtpp,youtint,nout, &
    & ioptder,iextrapol,taus,nbc,ybc,iflag)
  ...
  print *,'];'
  print *,' out=[ '
  write(*,'(1p2e25.15)') (xout(i), yout(i), i=1,nout)
  print *,'];'

  ! test using splibnd
  ! first calculated spline function and 2nd der: yinspl and yinsplpp on xin
  call system_clock(icount)
  call interpos(xin,yin,yinspl,yinsplpp,nin,xin,yout,youtp,youtpp,youtint,1, &
  ...
  call system_clock(icount)
  CALL SPLIBNDA2(xin,yinspl,yinsplpp,nin,xout,yout, youtp, youtpp,youtint,nout,iextrapol,ioptder)
      &      iextrapol)
  do i=1,nout
  ...
  ybc(1)=xin(nin)-xin(1)
  sigma=taus
  call cbsplgenp0(xin(1:nin-1),yin(1:nin-1),yinspl(1:nin-1),yinsplpp(1:nin-1),nin-1,xout,yout,youtp,youtpp,youtint,nout, &
  ...
  call system_clock(icount, irate, icmax)
  write(7,*) ' icount, irate, icmax= ',icount, irate, icmax
  call interpos('extrap_quad',xin,yin,nin,xout(1),yout(1),youtint=youtint(1),nout=1)
  print *,'% taus= ',taus
  stop
  call system_clock(icount2, irate, icmax)
  write(7,*) ' icount2, irate, icmax, icount2-icount= ',icount2, irate, icmax, icount2-icount

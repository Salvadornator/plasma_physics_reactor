...
c   pointers for matlab output arguments
      integer*8 pyout, pyoutp, pyoutpp
...
c   1. defaults values
c
c%OS      print *,' nrhs= ',nrhs
c%OS      print *,'prhs= ',prhs(1),prhs(2),prhs(3)
      nbyt_tot=0
      taus = 0.
c%OS      print *,' nbyt_tot= ',nbyt_tot
c
c   2. Input arguments
c
...
c
      ...
      inttype = mod(kopt,10)
      iextrapo = kopt/nel
c%OS      print *,' iextrapo, inttype = ',iextrapo,inttype 
c
...
c%OS      print *,'nincol, ninrow= ',nincol, ninrow
c   input 1D arrays in row or column
      nin = max(ninrow,nincol)
      nbytes = 8*nin
c%OS      print *,nbytes,'  salut'
...
c%OS        print *,' iybclenint8= ',iybclenint8
...
c%OS      print *,' noutrowint8, noutcolint8= ',noutrowint8, noutcolint8
c%OS      print *,' noutrow, noutcol= ',noutrow, noutcol
      plhs(1) = mxCreateFull(noutrowint8,noutcolint8,nelint8)
c%OS      print *,plhs(1)
      pyout = mxGetPr(plhs(1))
c%OS      print *,'nbytes, nbytes8= ',nbytes, nbytes8
...
c%OS        print *,' ioptder,iextrapo,inttype= ',ioptder,iextrapo,inttype
c%OS        print *,'xin,yin,nin,xout',xin,yin,nin,xout
...
c%OS      print *,'yout(1:10)',yout(1:10)
c%OS      print *,'pyout= ',pyout
c%OS      print *,'noutint8= ',noutint8
...
c      call mxfree(iptr_yout)
c      call mxfree(iptr_youtp)
c      call mxfree(iptr_youtpp)
c
cc      call mxfree(pyoutp)
cc      call mxfree(psig)
c
      return
      end
...
c
      call mxfree(iptr_pynew)
      call mxfree(iptr_pyinpp)
      call mxfree(iptr_pamat)
      call mxfree(iptr_pwork)
      call mxfree(iptr_kwork)
      END
c
c.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.     
c
...
c
c%OS
c%OS      print *,'isym= ',isym
      IF (ISYM .EQ. 1) THEN
        CALL SPBTRF('U',N,IUP,PAMAT,IDIMA,INFO)
c%OS        print *,pamat(10,1), info,N,IUP,IDIMA
c%OS        print *,'N,IUP,IDIMA= ',N,IUP,IDIMA
      ELSE
        CALL SGBTRF(N,N,IDOWN,IUP,PAMAT,IDIMA,KPM2(1,0),INFO)
      ENDIF
      IF (INFO .EQ. 0) THEN
        IF (ISYM .EQ. 1) THEN
c%OS        print *,'IRHS,N,IDIMA,IDIMRHS= ',IRHS,N,IDIMA,IDIMRHS
c%OS        print *,'PAMAT= ',PAMAT
c%OS        print *,'PYINPP= ',PYINPP
c%OS        print *,'N,IUP,IRHS,IDIMA,IDIMRHS,INFO= ',
c%OS     +  N,IUP,IRHS,IDIMA,IDIMRHS,INFO
...
*
c%OS      print *,'UPLO, TRANS, DIAG, N, K, A, LDA, X, INCX= ', 
c%OS     + UPLO, TRANS, DIAG, N, K, A(1:2,1:N),LDA, X(1:N), INCX
      IF( INCX.LE.0 )THEN
         KX = 1 - ( N - 1 )*INCX
...
*
     ...
     $   RETURN
*
c%OS      print *,'  UPLO, N, KD, NRHS, AB, LDAB, B, LDB, INFO',  
c%OS     + UPLO, N, KD, NRHS, AB(1,1), LDAB, B(1,1), LDB, INFO
c%OS      print *,'AB= ',AB(1:2,1:N)
c%OS      print *,' B= ',B(1:N,1)
      IF( UPPER ) THEN
*
...
c
      ...
      integer*8 function malloc_f(nbytes,infomalloc)
      implicit none
      integer nbytes, nbyt_tot, maxmem,infomalloc
      integer*8 malloc, nbytesint8, mxmalloc
...
c%OS        print *,' nbyt_tot , nbytes= ',nbyt_tot , nbytes
c%OS        print *,' malloc bytes so far: ',nbyt_tot/1e6,' Mb'
C   SUN
        nbytesint8 = int(nbytes,8)
c%OS        print *,'nbytesint8= ',nbytesint8
c        malloc_f = malloc(nbytesint8)
        malloc_f = mxmalloc(nbytesint8)
C   IBM
CCC        malloc_f = malloc(%val(nbytes))
c
        infomalloc=0
      endif
c%OS      print *,malloc_f
